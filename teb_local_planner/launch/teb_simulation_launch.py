...
def generate_launch_description():
    ...
    declare_map_yaml_cmd = launch.actions.DeclareLaunchArgument(
        'map',
        default_value=[launch_dir, '/turtlebot3_world.yaml'],
        description='Full path to map file to load')

    declare_params_file_cmd = launch.actions.DeclareLaunchArgument(
        'params',
        default_value=[launch_dir, '/teb_params.yaml'],
    ...
    start_teb_cmd = launch.actions.ExecuteProcess(
        cmd=[
            os.path.join(
                get_package_prefix('teb_local_planner'),
                'bin/teb_controller'),
            ['__params:=', configured_params]],
        cwd=[launch_dir], output='screen')
